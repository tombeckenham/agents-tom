/**
 * Wire envelope types for `@cloudflare/ai-chat-vercel`.
 *
 * The CF_AGENT_* framing is identical to the legacy `@cloudflare/ai-chat`
 * package; only the body of `CF_AGENT_USE_CHAT_RESPONSE` differs. Server-
 * side that body carries one AG-UI event JSON per frame (no `data: ` SSE
 * prefix on the wire — the prefix is only present inside an SSE response
 * body, not the WebSocket envelope). Client-side the transport parses each
 * body as `JSON.parse(body) as AGUIEvent` and projects to `UIMessageChunk`.
 */
